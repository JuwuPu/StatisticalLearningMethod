...
class SVM(object):
    ...
    def predict(self, test, test_labels):
        ...
        pred = []
        for item in test:
            temp = sum(np.inner(item, self.features) * self.labels * self.alpha) + self.b
...
def SMO_train(features, labels, max_passes=50, C=10, tol=0.1):
    """
    ...
    while passes < max_passes:
        ...
        for i in range(len(labels)):
            ...
            temp_list.pop(i)
            error_i = sum(np.inner(features[i], features) * alpha * labels) + b - labels[i]
            if (labels[i] * error_i < -tol and alpha[i] < C)\
                    or (labels[i] * error_i > tol and alpha[i] > 0):
                j = random.choice(temp_list)
                error_j = sum(np.inner(features[j], features) * alpha * labels) + b - labels[j]
                ...
                if L == H:
                    continue
                eta = 2 * np.inner(features[i], features[j]) - \
                    np.inner(features[i], features[i]) - np.inner(features[j], features[j])
                if eta >= 0:
                    continue
                if alpha[j] > H:
                    alpha[j] = H
                ...
                b1 = b - error_i - labels[i]*(alpha[i]-temp_alpha_i_old)* \
                     np.inner(features[i],features[i]) - labels[j]*(alpha[j]-temp_alpha_j_old) * \
                     np.inner(features[i],features[j])
                b2 = b - error_j - labels[i]*(alpha[i]-temp_alpha_i_old) * \
                     np.inner(features[i],features[j]) - labels[j]*(alpha[j]-temp_alpha_j_old) * \
                     np.inner(features[j],features[j])
                if 0 < alpha[i] < C:
                    b = b1
                elif 0 < alpha[j] < C:
                    b = b2
